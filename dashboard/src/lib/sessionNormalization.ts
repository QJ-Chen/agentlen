...
function normalizeChildLLMCall(
  ...
  return {
    ...
    outputTokens,
    totalTokens,
    cacheReadTokens: Math.max(0, child.cache_read_tokens || 0),
    cacheCreationTokens: Math.max(0, child.cache_creation_tokens || 0),
...
export function buildLLMCalls(record: RawSessionRecord | RawSubagentLog, recordStartTime: number) {
  return (record.llm_calls || []).flatMap((call, idx) => {
    ...
    return [{
      ...
      outputTokens,
      totalTokens,
      cacheReadTokens: Math.max(0, call.cache_read_tokens || 0),
      cacheCreationTokens: Math.max(0, call.cache_creation_tokens || 0),
